...
def send_telegram_text(msg):
    ...
    if len(msg) > 4000:
        ...
        for part in parts:
            try:
                session.post(url, json={'chat_id': CHAT_ID, 'text': part, 'parse_mode': 'Markdown', 'disable_web_page_preview': True}, timeout=10)
                time.sleep(0.5)
    ...
    try:
        res = session.post(url, json={'chat_id': CHAT_ID, 'text': msg, 'parse_mode': 'Markdown', 'disable_web_page_preview': True}, timeout=10)
        if res.status_code != 200:
            session.post(url, json={'chat_id': CHAT_ID, 'text': msg, 'disable_web_page_preview': True}, timeout=10)
    except Exception as e:
...
def send_telegram_photo(photo_path, caption):
    ...
    if len(caption) > 1000:
        try:
            with open(photo_path, 'rb') as photo:
                session.post(url, data={'chat_id': CHAT_ID, 'caption': "📸 GEMINI ALFA ANALİZ GRAFİĞİ"}, files={'photo': photo}, timeout=15)
            time.sleep(1)
    ...
    try:
        with open(photo_path, 'rb') as photo:
            res = session.post(url, data={'chat_id': CHAT_ID, 'caption': caption, 'parse_mode': 'Markdown'}, files={'photo': photo}, timeout=15)
        if res.status_code != 200:
            send_telegram_text(caption)
    except Exception as e:
...
def log_new_signal(symbol, entry_price, rsi, res_1h):
    ...
    fieldnames = ['timestamp_utc', 'symbol', 'entry_price', 'rsi', 'res_1h', 'outcome', 'outcome_pct', 'resolved']
    try:
...
def get_market_candidates():
    ...
    for t in usdt_swaps:
        ...
        res_1h, is_fresh_breakout, breakout_status = check_fresh_breakout_or_resistance(df)
...
def main():
    ...
    for coin in candidates:
        ...
        coin_text = f"📍 *{symbol}*\n"
        coin_text += f"• Fiyat: `{coin['current_price']}` | 24s Değişim: `%{coin['chg']:.1f}`\n"
        coin_text += f"• 1H RSI: `{coin['rsi']:.1f}` | 1H Direnç: `{coin['res_1h']}`\n"
        ...
        log_new_signal(symbol, coin['current_price'], coin['rsi'], coin['res_1h'])
